...
def get_ucb(total, success, num_samples):
    """
    ...
    ucb = average + math.sqrt((2 * math.log(num_samples)) / success)
    return ucb
...
def recommend_words_ucb(user_type, num_recommendations=10):
    ...
    for word_doc in words:
        word = word_doc["word"]
        params = word_doc["params"]
        ucb = get_ucb(params["successes"] + params["failures"], params["successes"], total_samples)
...
class humanFeedback(Resource):
    ...
    def post(self, choice_word):
        recommended_words = recommend_words_ucb(ns.payload['user_type'], num_recommendations=10)
