...
def get_related_words(word, limit=1000):
    ...
    for item in data['edges']:
        if item['rel']['label'] == 'RelatedTo':
            related_word = item['start']['label'].lower() if item['start']['label'].lower() != word else item['end'][
                'label'].lower()
...
def select_word(word, selected_words, user_type):
    ...
    result = {key: {'probability': value['probability'] / total, 'rewards': value['rewards']} for key, value in
              y.items()}
...
def human_feedback(center_word, choice_word, selected_words):
    if choice_word == 'none':
        ...
        update_reward(center_word,
                      1)  # positive reward for the center_word as the user chose one of the recommendations
